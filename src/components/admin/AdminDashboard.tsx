...
import { supabase, portfolioApi, type Category, type Subcategory, type PortfolioItem, type Enquiry, type Resume } from "@/lib/supabase";
...
const AdminDashboard = ({ initialCategories, initialSubcategories }: AdminDashboardProps) => {
  ...
  useEffect(() => {
    ...
    if (authStatus === 'true') {
      ...
      // Only load other data (portfolio items, enquiries, resumes) if authenticated,
      // as categories and subcategories are now passed as initial props.
      loadData(false); // Pass false to indicate not to fetch categories/subcategories again
    } else {
  ...
  const handleLogin = async () => {
    ...
    try {
      if (username === "admin" && password === "admin123") {
        ...
        loadData(true); // Fetch all data after successful login (including categories/subcategories in case they were empty)
        toast({
  ...
  const loadData = async (fetchAllCategoriesAndSubcategories: boolean) => {
    ...
    try {
      ...
      // Adjust based on whether categories/subcategories were fetched
      if (fetchAllCategoriesAndSubcategories) {
  ...
  const signOut = () => {
    ...
    setUsername("");
    setPassword("");
    setCategories(initialCategories); // Reset to initial props on logout
    setSubcategories(initialSubcategories); // Reset to initial props on logout
    setPortfolioItems([]);
    setEnquiries([]);
    setResumes([]);
    setLoading(false);
